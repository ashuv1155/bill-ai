...
export default function AdminDashboard() {
  ...
  // Load Admin Data from LocalStorage (or initial setup)
  useEffect(() => {
    if (typeof window !== "undefined" && user) {
      // 1. Setup User Records
      ...
      if (storedUsers) {
        // Sync current logged in user details dynamically
        ...
        setAdminUsers(synced);
        localStorage.setItem("admin_users_db", JSON.stringify(synced));
      } else {
        const initialUsers: AdminUserRecord[] = [
          { uid: user.uid, name: user.displayName || "You", email: user.email, joined: "2026-06-01", tier: currentSavedTier, scans: currentSavedScans, status: "Active" },
        ...
        localStorage.setItem("admin_users_db", JSON.stringify(initialUsers));
      }

      // 2. Setup Audit Documents
      ...
      if (storedDocs) {
        ...
        const initialDocs: AdminAuditDoc[] = [
          ...
          { id: "doc-5", ownerEmail: user.email, vendorName: "Duplicate Supplier Corp", date: "2026-06-19", category: "Office Expense", amount: 450.00, compliance: "Flagged", flagDetails: "Duplicate invoice reference detected" },
        ];
  ...
  // Update Plan Tier dynamically
  const handleUpdateTier = (uid: string, email: string, newTier: any) => {
    ...
    // If updating current logged in user, propagate to AuthContext
    if (email === user.email) {
      updateSubscriptionTier(newTier);
    }
  };

  // Reset usage scans to 0
  const handleResetScans = (uid: string, email: string) => {
    const updated = adminUsers.map((u) => {
      if (u.uid === uid) {
        return { ...u, scans: 0 };
      }
    ...
    // Propagate to current user local storage
    if (email === user.email) {
      localStorage.setItem("saas_scan_count", "0");
      // Hard refresh page to reload sidebar state
